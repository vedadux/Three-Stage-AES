#include <verilated.h>
#include "Vmasked_aes_sbox_fwd.h"
...
int main(int argc, char** argv) 
{
    ...
    Vmasked_aes_sbox_fwd* dut = new Vmasked_aes_sbox_fwd;
    ...
    printf("uint8_t aes_sbox[256] = {\n    ");
    for (uint32_t input = 0; input < 256; input += 1)
    {
        std::printf("0x%02lx, ", aes_sbox(input));
        if (input % 16 == 15) printf("\n    ");
    }
    printf("};\n");

    printf("uint8_t front_basis[256] = {\n    ");
    for (uint32_t input = 0; input < 256; input += 1)
    {
        std::printf("0x%02lx, ", gf_256_switch_basis(input, A2X));
        if (input % 16 == 15) printf("\n    ");
    }
    printf("};\n");
    ...
    {
        uint32_t expected = aes_sbox(input);
